...
export function calculateHonestyScore(build: NormalizedBuild, conditions: Condition[]) {
  ...
  return { score, label: score >= 8 ? "Low inflation risk" : score >= 5 ? "Needs review" : "High inflation risk", factors, explanation: "This is a transparency score for configured DPS assumptions, not a power rating. It starts at 10 and loses points when conditions are unsupported or encounter-limited; temporary damage is not automatically treated as fake." };
}
