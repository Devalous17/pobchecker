...
import { enrichGemAssets } from "@/src/features/poeninja/assets";
export async function POST(request: Request) {
  try { const body = await request.json(); const imported = await fetchPobRaw(body?.url); const build = parsePobXml(decodePobExport(imported.raw)); build.sourceAssets = await enrichGemAssets(build.sourceAssets); if (imported.fixture) build.diagnostics.push("Offline example fixture used because the local runtime could not reach pobb.in."); return NextResponse.json(buildReport(build, detectConditions(build))); }
