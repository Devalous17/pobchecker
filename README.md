...
Imported values are authoritative values from the PoB export, not invented estimates. Alternate peak, burst, sustained, initial, bossing, and mapping scenarios still require the isolated Headless PoB worker to recalculate each configuration safely.
...
The official Path of Building engine is kept separate from the TypeScript importer. The worker clones a pinned Path of Building Community revision and uses its headless wrapper for future scenario recalculation; the application does not reimplement PoB's calculation engine.
