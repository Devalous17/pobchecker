...
local allowed = {
  ...
  conditionRecentlyKilled = true, buffOnslaught = true,
}
...
local function calculate(request)
  ...
  local inputs = build.configTab and build.configTab.input or {}
  for key, value in pairs(request.scenario or {}) do
    if allowed[key] and type(value) == "boolean" and inputs[key] then inputs[key].enabled = value end
  end
  ...
  return {
    ...
    offence = { totalDPS = numberOrNil(output.TotalDPS), totalDot = numberOrNil(output.TotalDot), averageDamage = numberOrNil(output.AverageDamage), speed = numberOrNil(output.Speed) },
