...
function DefenceLayerTarget({ label, current, target, suffix = "", note }: { label: string; current: number | null; target?: number; suffix?: string; note: string }) {
  ...
  return <div className={`figma-defence-layer-card ${informational ? "is-informational" : met ? "is-met" : "is-neutral"}`}><div><strong>{label}</strong><span>{current === null ? "Not exported" : `${compactNumber(current)}${suffix}`}</span></div><small>{informational ? "Imported value · no fixed minimum" : `Recommended minimum: ${compactNumber(target)}${suffix}`}</small><p>{informational ? (current === null ? "No imported value was found." : "Useful when this recovery layer is part of the build; no universal minimum is applied.") : met ? "Recommended minimum reached." : current === null ? "No imported value was found." : `Below the recommended minimum by about ${compactNumber(target - current)}${suffix}.`}</p><em>{note}</em></div>;
}
...
function OffenceTips({ report }: { report: Report }) {
  const fullDps = numericStat(report.build.importedStats, "fullDps");
  const hitDps = numericStat(report.build.importedStats, "totalDps");
  ...
  const mainSetup = report.build.skillSetups.find((setup) => setup.mainActiveSkill || setup.includeInFullDPS) ?? report.build.skillSetups[0];
  const sixLinkDetected = report.build.skillSetups.some((setup) => setup.gems.filter((gem) => gem.enabled && !gem.provided).length >= 6) || report.build.equippedItems.some((item) => Number.parseInt(item.links ?? "", 10) >= 6);
...
function DefenceTipsV2({ report }: { report: Report }) {
  ...
  const elementalMaximum = ["fireMaximumHit", "coldMaximumHit", "lightningMaximumHit"].map((key) => numericStat(stats, key)).filter((value): value is number => value !== null);
  const elementalHit = elementalMaximum.length ? Math.min(...elementalMaximum) : null;
  return <FigmaPanel title="General defence tips" eyebrow="Practical normal T16 starting points" className="figma-page-panel figma-tips-panel"><p className="figma-explainer">These are starting targets for ordinary white Tier 16 maps, not universal guarantees. Rare mods, altars, damage conversion, critical hits, and league mechanics can demand much more.</p><div className="figma-target-grid"><TargetMetric label="Elemental resistance" current={numericStat(stats, "fireResistance") !== null && numericStat(stats, "coldResistance") !== null && numericStat(stats, "lightningResistance") !== null ? Math.min(numericStat(stats, "fireResistance")!, numericStat(stats, "coldResistance")!, numericStat(stats, "lightningResistance")!) : null} target={75} suffix="%" note="Fire, cold, and lightning should all reach the normal cap." /><TargetMetric label="Chaos resistance" current={numericStat(stats, "chaosResistance")} target={0} suffix="%" note="Non-negative chaos resistance is a useful minimum; higher is safer." /><TargetMetric label="Physical maximum hit" current={numericStat(stats, "physicalMaximumHit")} target={10_000} note="A rough ordinary-map starting point before armour, conversion, and map mods." /><TargetMetric label="Elemental maximum hit" current={elementalHit} target={20_000} note="Uses the weakest exported fire, cold, or lightning maximum-hit value." /><TargetMetric label="Chaos maximum hit" current={numericStat(stats, "chaosMaximumHit")} target={5_000} note="Chaos resistance and maximum hit should be considered together." /><TargetMetric label="Spell suppression" current={numericStat(stats, "spellSuppression")} target={100} suffix="%" targetKind="Recommended target" note="Full suppression is strongly recommended when the build uses this layer, but it is not mandatory for every build." /></div><ul className="figma-tip-list"><li>Fix the largest red gap first: uncapped resistance, very low maximum hit, or a missing recovery layer usually matters more than small DPS gains.</li><li>Build at least two reliable defensive layers: a hit pool plus mitigation, avoidance, block, suppression, recovery, conversion, or damage reduction.</li><li>Do not count a guard skill, flask, charge, or recently effect as permanent unless the build can keep it active reliably.</li><li>Maximum hit is not the same as immunity. Recovery, ailment protection, movement, and dangerous map modifiers still matter.</li></ul></FigmaPanel>;
}
