...
function DefenceLayerTarget({ label, current, target, suffix = "", note }: { label: string; current: number | null; target: number; suffix?: string; note: string }) {
  ...
  return <div className={`figma-defence-layer-card ${met ? "is-met" : "is-neutral"}`}><div><strong>{label}</strong><span>{current === null ? "Not exported" : `${compactNumber(current)}${suffix}`}</span></div><small>Recommended minimum: {compactNumber(target)}{suffix}</small><p>{met ? "Recommended minimum reached." : current === null ? "No imported value was found." : `Below the recommended minimum by about ${compactNumber(target - current)}${suffix}.`}</p><em>{note}</em></div>;
}

function DefenceLayerTargets({ stats }: { stats: Stats }) {
  return <section className="figma-defence-layer-section"><div className="figma-defence-layer-heading"><span>OTHER DEFENSIVE LAYERS</span><strong>Recommended minimums</strong><p>These layers stay neutral until the imported value reaches a practical starting point. They are recommendations, not universal requirements.</p></div><div className="figma-defence-layer-grid"><DefenceLayerTarget label="Life regeneration" current={numericStat(stats, "lifeRegen")} target={100} suffix="/s" note="Reliable recovery between hits." /><DefenceLayerTarget label="Energy Shield regeneration" current={numericStat(stats, "energyShieldRegen")} target={100} suffix="/s" note="Useful when ES is part of the build's main hit pool." /><DefenceLayerTarget label="Life leech" current={numericStat(stats, "lifeLeechRate")} target={100} suffix="/s" note="Recovery while actively damaging enemies." /><DefenceLayerTarget label="Energy Shield leech" current={numericStat(stats, "energyShieldLeechRate")} target={100} suffix="/s" note="Recovery layer for ES-based or hybrid builds." /><DefenceLayerTarget label="Mana leech" current={numericStat(stats, "manaLeechRate")} target={50} suffix="/s" note="Helps sustain skill costs during combat." /><DefenceLayerTarget label="Attack block" current={numericStat(stats, "block")} target={50} suffix="%" note="A practical avoidance starting point." /><DefenceLayerTarget label="Spell block" current={numericStat(stats, "spellBlock")} target={50} suffix="%" note="A useful secondary spell-avoidance layer." /><DefenceLayerTarget label="Spell suppression" current={numericStat(stats, "spellSuppression")} target={100} suffix="%" note="Strongly recommended when this layer is part of the build." /></div></section>;
}
