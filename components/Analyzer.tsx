...
function TargetMetric({ label, current, target, suffix = "", note, targetKind = "Starting target" }: { label: string; current: number | null; target: number; suffix?: string; note: string; targetKind?: string }) {
  const gap = current === null ? null : Math.max(0, target - current);
  return <div className={`figma-target-metric ${current !== null && current >= target ? "is-met" : "is-gap"}`}><div><strong>{label}</strong><span>{current === null ? "Unavailable" : `${compactNumber(current)}${suffix}`}</span></div><small>{targetKind}: {compactNumber(target)}{suffix}</small><p>{current === null ? "This value was not exported by PoB." : gap !== null && gap > 0 ? `Improve by about ${compactNumber(gap)}${suffix} to reach this benchmark.` : "Benchmark reached in the imported snapshot."}</p><em>{note}</em></div>;
}
